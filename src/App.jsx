...
const ringAlarm = () => {
  try {
    ...
    playBeep(ctx.currentTime + 2.5, 1320);
  } catch (e) {
...
function HomePage({ nav, toast }) {
  ...
  const handleRegister = async () => {
    if (!name.trim()) { toast("Enter your name", "⚠️"); return; }
    if (!email.trim() || !password.trim()) { toast("Enter email and password", "⚠️"); return; }
  ...
  const handleForgot = async () => {
    if (!email.trim()) { toast("Enter your email first", "⚠️"); return; }
    setLoading(true);
    try {
      ...
      toast("Could not send reset email. Check address.", "❌");
    }
    setLoading(false);
  };
  ...
  const labelStyle = {
    display: "block", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.6)",
      ...
      <div style={{
        ...
        <div className="fade-up" style={{ textAlign: "center", marginBottom: 36 }}>
          <h1 style={{
            ...
            fontSize: "clamp(44px, 14vw, 68px)", fontWeight: 800, lineHeight: 1.0,
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, marginTop: 8, fontStyle: "italic" }}>
            {mode === "login" ? "Sign in to order" : mode === "register" ? "Create your account" : "Reset your password"}
          </p>
        </div>
        ...
        <div className="fade-up-1" style={{
          ...
          {mode === "register" && (
            <div style={{ marginBottom: 18 }}>
              ...
              <input
                style={inputStyle} type="text" placeholder="e.g. John Adeyemi"
          ...
          {mode !== "forgot" && (
            <div style={{ marginBottom: 24 }}>
              ...
              <input
                style={inputStyle} type="password" placeholder={mode === "register" ? "At least 6 characters" : "Your password"}
          ...
          {mode === "register" && (
            <button className="btn btn-fire btn-full" onClick={handleRegister} disabled={loading}
              style={{ padding: 15, fontSize: 15, borderRadius: 12, marginBottom: 16 }}>
              {loading ? "Creating account…" : "Create Account →"}
            </button>
          )}
          {mode === "forgot" && (
            <button className="btn btn-fire btn-full" onClick={handleForgot} disabled={loading}
              style={{ padding: 15, fontSize: 15, borderRadius: 12, marginBottom: 16 }}>
          ...
          {mode === "login" && (
            <>
              ...
              <button onClick={() => setMode("register")} style={{
                ...
                cursor: "pointer", color: "#fff", fontSize: 14, padding: "13px 0",
                borderRadius: 12, fontFamily: "var(--font-h)", fontWeight: 700,
              }}>
                Create Account
              </button>
            </>
  ...
  useEffect(() => {
    ...
    if (prevStatus.current && prevStatus.current !== order.order_status) {
      const messages = {
        ...
        ready:     { title: "🎉 Order Ready!", body: "Your noodles are ready for pickup!" },
        completed: { title: "✅ Order Completed", body: "Thanks for ordering from Joy's Kitchen!" },
      };
...
function VendorLogin({ nav, toast }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      toast(e.code === "auth/invalid-credential" ? "Wrong email or password" : "Login failed.", "❌");
    }
    setLoading(false);
  };
    ...
    <div style={{ minHeight: "100dvh", position: "relative", background: "#0e0905" }}>
      ...
      <div style={{
        ...
        <div className="fade-up" style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, marginTop: 8, fontStyle: "italic" }}>
            {mode === "login" ? "Vendor Sign In" : "Reset Password"}
          </p>
        </div>
      ...
      <div className="topbar">
        <div className="topbar-inner" style={{ maxWidth: 900 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {queue.length > 0 && <span className="badge pulse-ring" style={{ background: "var(--fire)", color: "#fff", fontSize: 12, padding: "5px 12px" }}>{queue.length} orders</span>}
          </div>
        </div>
      </div>
...
function VendorPricesPage({ nav, toast }) {
  ...
  // Raw string state for price inputs so typing "0" or "" doesn't get coerced prematurely
  const [menuPriceRaw, setMenuPriceRaw] = useState({});
  const [addonPriceRaw, setAddonPriceRaw] = useState({});
  const [saving, setSaving] = useState(false);
  const [activeTab, setActiveTab] = useState("menu"); // "menu" | "addons"
  const [showPrices, setShowPrices] = useState(true); // toggle price column visibility
  ...
  useEffect(() => {
    if (addons && !editAddons) {
      ...
      const raw = {};
      addons.forEach(i => { raw[i.id] = String(i.price ?? ""); });
      setAddonPriceRaw(raw);
    }
  }, [addons]);
  ...
  const ColHeader = () => (
    <div style={{ display: "grid", gridTemplateColumns: showPrices ? "1fr 120px" : "1fr", gap: 10, marginBottom: 10, padding: "0 2px" }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.07em" }}>Item Name</span>
      {showPrices && <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.07em", textAlign: "right" }}>Price (₦)</span>}
    </div>
  );
    ...
    <div className="page" style={{ background: "var(--cream)" }}>
      ...
      <div className="wrap-wide" style={{ paddingTop: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "0 2px" }}>
          <span style={{ fontSize: 13, color: "var(--muted)", fontWeight: 600 }}>Show price fields</span>
          <button
            onClick={() => setShowPrices(p => !p)}
            style={{
              padding: "5px 14px", borderRadius: 20, border: "2px solid var(--fire)",
              background: showPrices ? "var(--fire)" : "transparent",
              color: showPrices ? "#fff" : "var(--fire)",
              fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "var(--font-b)",
              transition: "all 0.15s",
            }}
          >
            {showPrices ? "ON" : "OFF"}
          ...
          {!showPrices && (
            <span style={{ fontSize: 11, color: "var(--muted)", fontStyle: "italic" }}>
              Prices hidden from this view (still saved as ₦0)
            </span>
          )}
        </div>
        ...
        {activeTab === "menu" && (
          <div className="card" style={{ padding: "20px 16px", marginBottom: 20 }}>
            ...
            {(editMenu || menu).map((item) => {
              ...
              return (
                <div key={item.id} style={{ display: "grid", gridTemplateColumns: showPrices ? "1fr 120px" : "1fr", gap: 10, marginBottom: 10, alignItems: "center" }}>
                  <input

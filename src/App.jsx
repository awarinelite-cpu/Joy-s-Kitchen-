...
function HomePage({ nav, savedOrderId }) {
  return (
    <div className="page" style={{ background: "var(--dark)", minHeight: "100dvh" }}>
      {/* Hero */}
      <div style={{
        padding: "56px 24px 40px", position: "relative", overflow: "hidden",
        background: "linear-gradient(160deg, #100804 0%, #1e1009 60%, #2a1510 100%)"
      }}>
        <div style={{ position: "absolute", top: -80, right: -80, width: 260, height: 260, borderRadius: "50%", background: "radial-gradient(circle, rgba(232,69,10,0.22) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -30, left: -50, width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,166,35,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />

        <div className="wrap" style={{ position: "relative" }}>
          <div className="fade-up" style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: 20, padding: "4px 12px", marginBottom: 16 }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#10b981", display: "inline-block", animation: "pulse 2s ease infinite" }} />
            <span style={{ fontSize: 11, fontWeight: 700, color: "#34d399", letterSpacing: "0.08em" }}>OPEN NOW</span>
          </div>

          <h1 className="fade-up-1" style={{ fontFamily: "var(--font-h)", color: "#fff", fontSize: "clamp(42px, 11vw, 64px)", fontWeight: 800, lineHeight: 1.0, marginBottom: 12 }}>
            Joy's<br /><span style={{ color: "var(--fire)" }}>Kitchen</span>
          </h1>
          <p className="fade-up-2" style={{ fontSize: "42px", marginBottom: 8, lineHeight: 1 }}>🍜</p>
          <p className="fade-up-2" style={{ color: "rgba(255,255,255,0.55)", fontSize: 15, marginBottom: 32, lineHeight: 1.6, maxWidth: 280 }}>
            Fresh noodles, cooked to order. Your way, every time.
          </p>
          <div className="fade-up-3" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button className="btn btn-fire" onClick={() => nav("menu")} style={{ fontSize: 15, padding: "15px 28px" }}>
              Order Now →
            </button>
            {savedOrderId && (
              <button className="btn btn-ghost" onClick={() => nav("order", { orderId: savedOrderId })}
      ...
      {/* Stats strip */}
      <div style={{ background: "rgba(255,255,255,0.04)", borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "18px 0" }}>
        <div className="wrap" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4 }}>
          {[["🔥", "Made Fresh"], ["⚡", "5–10 Min"], ["🏆", "First Paid First"]].map(([ic, lb]) => (
            <div key={lb} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, padding: "8px 0" }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.45)", textAlign: "center", textTransform: "uppercase", letterSpacing: "0.06em" }}>{lb}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "28px 0 20px" }}>
        <div className="wrap">
          <h2 style={{ fontFamily: "var(--font-h)", color: "#fff", fontSize: 22, marginBottom: 18 }}>
            Today's Menu
          </h2>
          {MENU.map((item, i) => (
            <div key={item.id} onClick={() => nav("customize", { noodleId: item.id })}
              ...
              style={{
                ...
                marginBottom: 10, borderRadius: 13, cursor: "pointer",
                background: item.id === "medium" ? "rgba(232,69,10,0.12)" : "rgba(255,255,255,0.04)",
                border: `1.5px solid ${item.id === "medium" ? "rgba(232,69,10,0.3)" : "rgba(255,255,255,0.07)"}`,
                transition: "all 0.2s", animationDelay: `${i * 0.08}s`
              }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(232,69,10,0.5)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = item.id === "medium" ? "rgba(232,69,10,0.3)" : "rgba(255,255,255,0.07)"; }}
            >
              <span style={{ fontSize: 36 }}>🍜</span>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  <span style={{ fontFamily: "var(--font-h)", color: "#fff", fontSize: 17, fontWeight: 700 }}>{item.name}</span>
                  <span className="badge" style={{ background: item.id === "medium" ? "var(--fire)" : "rgba(255,255,255,0.08)", color: item.id === "medium" ? "#fff" : "rgba(255,255,255,0.5)", fontSize: 9 }}>{item.tag}</span>
                </div>
                <span style={{ color: "rgba(255,255,255,0.38)", fontSize: 12 }}>~{item.time} min · {item.serves}</span>
              </div>
